...
export default function CivicFixDemo() {
  ...
  const analyzeImage = async () => {
    ...
    try {
      ...
      alert('Failed to analyze image. Make sure the backend is running at http://localhost:8000');
    } finally {
      setAnalyzing(false);
    }
  };
  ...
  const testImages = [
    "https://www.theaa.com/~/media/the-aa/article-summaries/driving-advice/potholes/pothole-main.jpg",
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-blue-300 mb-2">
                Paste Image URL or select a test case
              </label>
              <input
                type="text"
                value={imageUrl}
                ...
                className="w-full px-4 py-3 bg-white/5 border border-white/20 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-white placeholder-gray-500"
              />
            </div>
            ...
            <div className="flex gap-3">
              {testImages.map((url, idx) => (
                <button
                  ...
                  className="flex-1 px-4 py-2 bg-blue-500/20 hover:bg-blue-500/30 border border-blue-500/30 rounded-lg text-sm transition-all"
                >
                  Test Image {idx + 1}
                </button>
              ))}
